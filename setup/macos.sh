...
# Modified by Adam Bergman
# Run without downloading:
# curl https://raw.githubusercontent.com/adambrgmn/dotfiles/master/setup/macos.sh | bash
...
echo "🍺 brew cask installing applications"
brew cask install \
  alfred \
  visual-studio-code \
  iterm2 \
...
mkdir -p ~/.ssh
ssh-keygen -t rsa -b 4096 -C "adam@fransvilhelm.com"
echo "Host *\n AddKeysToAgent yes\n UseKeychain yes\n IdentityFile ~/.ssh/id_rsa" | tee ~/.ssh/config
eval "$(ssh-agent -s)"
echo "👉 run 'pbcopy < ~/.ssh/id_rsa.pub' and paste that into GitHub before continuing"
read -n1 -r -p "press any key when done\n" key
...
echo "📥 setting up ellipsis"
export ELLIPSIS_USER=adambrgmn
curl ellipsis.sh | PACKAGES='dotfiles' sh
